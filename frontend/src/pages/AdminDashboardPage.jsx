...
import { getUsers, updateUserStatus, getRiderReviews } from '../services/adminApi'
...
export default function AdminDashboardPage() {
  ...
  // Rider feedback: the individual reviews, the per-rider scorecard, and the
  // rider currently filtered on ('' = everyone).
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (tab === 'rider reviews') { const response = await getRiderReviews({ rider_id: riderFilter || undefined, page }); setRiderReviews(response.data.reviews); setRiderScores(response.data.riders); setHasMore(page < response.data.pagination.total_pages) }
    } catch (err) { setError(errorMessage(err)) }
  }, [tab, role, page, riderFilter])
  ...
  return <main className="page-shell py-9"><div className="dashboard-heading"><p className="eyebrow mb-2">Keep the good food moving</p><h1 className="section-heading">Cravio control room 🍊</h1></div><BusinessSummary />
    ...
    {/* Rider feedback is admin-only by construction: GET /api/admin/rider-reviews is the
        only endpoint in the API that returns these rows, so there is no version of this
        panel a customer, owner or rider could reach even by calling the API directly. */}
    {tab === 'rider reviews' && <section className="space-y-6">
      <div className="surface p-5"><h2 className="text-xl font-bold mb-1">🛵 Rider scorecard</h2><p className="text-sm muted mb-4">Averages across every delivery a customer rated. Worst average first — that is the row worth acting on. Riders never see any of this.</p>{!riderScores.length ? <p className="muted">No rider has been rated yet.</p> : <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b"><th className="p-3">Rider</th><th className="p-3">Average</th><th className="p-3">Ratings</th><th className="p-3">1–2 star</th><th className="p-3">Action</th></tr></thead><tbody>{riderScores.map(rider => <tr key={rider.rider_id} className="border-b"><td className="p-3"><strong>{rider.rider_name}</strong>{!rider.is_active && <span className="pill bg-red-50 text-red-900 ml-2">Suspended</span>}</td><td className="p-3"><StarRating rating={Number(rider.average_rating)} /></td><td className="p-3">{rider.review_count}</td><td className="p-3">{rider.low_rating_count > 0 ? <span className="pill bg-red-50 text-red-900">{rider.low_rating_count}</span> : '0'}</td><td className="p-3"><button className="btn-secondary" onClick={() => { setRiderFilter(String(rider.rider_id)); setPage(1) }}>See comments</button></td></tr>)}</tbody></table></div>}</div>
      <div className="surface p-5"><label className="field-label">Showing<select className="field !w-auto ml-3" value={riderFilter} onChange={event => { setRiderFilter(event.target.value); setPage(1) }}><option value="">Every rider</option>{riderScores.map(rider => <option key={rider.rider_id} value={rider.rider_id}>{rider.rider_name}</option>)}</select></label></div>
      <div className="space-y-4">{!riderReviews.length ? <p className="surface p-8 muted">No rider feedback to show.</p> : riderReviews.map(feedback => <article key={feedback.id} className="surface p-5"><div className="flex justify-between gap-4 flex-wrap"><div><p className="font-bold">🛵 {feedback.rider_name}{!feedback.rider_is_active && <span className="pill bg-red-50 text-red-900 ml-2">Suspended</span>}</p><p className="text-xs muted mt-2">Order #{feedback.order_id} · {feedback.restaurant_name} · from {feedback.customer_name}</p></div><div className="text-right"><StarRating rating={feedback.rating} /><p className="text-xs muted mt-2">{new Date(feedback.created_at).toLocaleString()}</p></div></div>{feedback.comment && <p className="text-sm mt-3 border-t pt-3">“{feedback.comment}”</p>}</article>)}</div>
    </section>}
    ...
    {['orders', 'users', 'rider reviews'].includes(tab) && <div className="flex justify-center items-center gap-4 mt-6"><button className="btn-secondary" disabled={page === 1} onClick={() => setPage(page - 1)}>Previous</button><span>Page {page}</span><button className="btn-secondary" disabled={!hasMore} onClick={() => setPage(page + 1)}>Next</button></div>}
  </main>
}
