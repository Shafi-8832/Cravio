...
import { getOrderDetails, cancelOrder, reviewOrder, submitPaymentReference, verifyPayment } from '../services/orderApi'
...
export default function OrderReceipt({ id, onChanged }) {
  ...
  const [review, setReview] = useState({ rating: 5, portion_accuracy: 'full', comment: '' })
  ...
  return <div className="p-5 bg-stone-50 rounded-b-2xl">
    ...
    {customer && order.status === 'delivered' && order.review_eligible && !reviewed && <form className="mt-6 border-t pt-5" onSubmit={event => { event.preventDefault(); action(async () => { await reviewOrder(order.id, review); setReviewed(true) }, 'Thank you! Your review has been published.') }}><h3 className="font-bold text-lg mb-3">How was your meal? 😋</h3><div className="grid sm:grid-cols-2 gap-3"><label className="text-sm">Rating<select className="field mt-1" value={review.rating} onChange={event => setReview({ ...review, rating: Number(event.target.value) })}>{[5, 4, 3, 2, 1].map(value => <option key={value} value={value}>{value} stars</option>)}</select></label><label className="text-sm">Portion size<select className="field mt-1" value={review.portion_accuracy} onChange={event => setReview({ ...review, portion_accuracy: event.target.value })}><option value="full">As expected</option><option value="slightly_less">Slightly less</option><option value="way_less">Much less</option></select></label></div><label className="field-label mt-3">Your feedback<textarea className="field mt-1" maxLength={1000} value={review.comment} onChange={event => setReview({ ...review, comment: event.target.value })} /></label><button disabled={busy} className="btn-primary mt-2">⭐ Submit review</button></form>}
  </div>
}
